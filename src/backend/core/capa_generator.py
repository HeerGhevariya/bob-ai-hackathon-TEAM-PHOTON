...
Uses deterministic template-based generation as the foundation.
Optionally enhanced by IBM watsonx.ai (Granite) for richer narrative
analysis when WATSONX_API_KEY is configured. The LLM never decides what
counts as a deviation — it only enriches the human-readable narrative.
"""
...
class CapaGenerator:
    ...
    regulatory standards. When IBM watsonx.ai is configured, the
    executive summary and root cause analysis are enhanced with
    LLM-generated narrative for richer regulatory context.
    
    The LLM only enhances presentation — it never decides what counts
    as a deviation, how severe it is, or what the risk score is.
    """
    ...
    def __init__(self):
        ...
        try:
            from .watsonx_client import is_watsonx_configured
            self._llm_available = is_watsonx_configured()
            if self._llm_available:
                print("🤖 CAPA Generator: watsonx.ai LLM enhancement enabled")
        except Exception:
            pass
    ...
    def generate_site_report(
        ...
        if severity_counts.get("major", 0) > 0:
            ich_class = "Major (TrialGuard classification rules)"
        elif severity_counts.get("minor", 0) > 2:
            ich_class = "Minor — recurring (TrialGuard classification rules)"
        else:
            ich_class = "Administrative (TrialGuard classification rules)"

        # Overall risk level comes from the risk profile tier when available;
        # the generator falls back to severity-count heuristics for the
        # markdown report only. The API endpoint overrides this with the
        # authoritative risk profile tier.
        if risk_profile:
            tier_map = {"critical": "Critical", "high": "High",
                        "medium": "Medium", "low": "Low"}
            overall_risk = tier_map.get(risk_profile.risk_tier.value, "High")
        ...
        if self._llm_available:
            try:
                from .watsonx_client import (
                    ...
                    enhance_root_cause_analysis,
                    generate_llm_recommendations,
                )

                # Enhance executive summary
                exec_summary = enhance_executive_summary(
                    site_id=site_id,
                    site_name=site_name,
                    template_summary=exec_summary,
                    severity_breakdown=dict(severity_counts),
                    deviation_types={k.value: v for k, v in type_counts.items()},
                    risk_score=risk_profile.risk_score if risk_profile else 0,
                    risk_tier=risk_profile.risk_tier.value if risk_profile else "unknown",
                    trend=risk_profile.trend_direction.value if risk_profile else "stable",
                )

                # Enhance root cause analysis
                root_cause = enhance_root_cause_analysis(
                    template_root_cause=root_cause,
                    deviation_descriptions=[d.description for d in deviations],
                ...
                print(f"⚠️  LLM enhancement failed (using template): {e}")
        ...
        # Append LLM recommendations section if available
        if llm_recommendations:
            full_report += (
                "\n\n---\n\n"
                "## 🤖 AI-Enhanced Recommendations\n\n"
                "*The following additional recommendations were generated by "
                "IBM watsonx.ai (Granite) based on the deviation patterns above. "
                "These supplement the standard corrective and preventive actions.*\n\n"
                f"{llm_recommendations}\n"
            )
...
## 3. Highest Severity (TrialGuard Classification Rules)

**Classification:** {ich_class}

Severity levels are TrialGuard's own classification rules, applied consistently
to every deviation. "Major" findings are proposed as important protocol deviations
requiring sponsor confirmation (per FDA Draft Guidance on Protocol Deviations,
December 2024, non-binding). Detection logic and thresholds are documented in
Appendix B. Regulatory non-compliance obligations are governed by ICH E6(R2)
Sections 4.5 and 5.20, and 21 CFR 312.56(b).

- **Major**: Proposed important protocol deviation — may significantly affect
  subject safety, data reliability, or scientific value. Requires sponsor
  confirmation and immediate corrective action.
- **Minor**: Represents protocol non-compliance unlikely to significantly affect
  safety or data reliability. Should be documented and corrected.
- **Administrative**: Documentation or process error with no safety or data
  integrity impact.
...
## 8. Detailed Findings (Appendix A Summary)
...
*This report was generated by TrialGuard AI. It is a DRAFT and must be reviewed
and approved by the Principal Investigator and Sponsor Medical Monitor before
submission to regulatory authorities. Severity classifications are TrialGuard's
rule-based system. Regulatory obligations are governed by ICH E6(R2) Sections
4.5 and 5.20, 21 CFR 312.56(b), and the FDA Draft Guidance on Protocol
Deviations (December 2024, non-binding).*
"""
        return md
